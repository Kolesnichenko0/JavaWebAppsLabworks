...
public class Train extends Transport {
    ...
    public Train(@NonNull String number, @NonNull String arrivalStation, @NonNull String departureStation, @NonNull MovementType movementType, @NonNull LocalTime departureTime, @NonNull Duration duration) {
        ...
        this.departureStation = validateStationName(departureStation, FieldName.DEPARTURE_STATION);
        this.arrivalStation = validateStationName(arrivalStation, FieldName.ARRIVAL_STATION);
    }

    public static String validateNumber(@NonNull String number) {
        if (number.isEmpty()) {
            throw new FieldValidationException("The number cannot be empty", FieldName.NUMBER);
        }
        ...
        if (!number.matches(NUMBER_REGEX)) {
            throw new FieldValidationException("The number should consist of 1 to 3 digits and {ІС+, ІС, РЕ, P, НЕ, НШ, НП}", FieldName.NUMBER);
        }

        return number;
    }

    public static String validateStationName(@NonNull String station, @NonNull FieldName stationType) {
        if (station.isEmpty()) {
            throw new FieldValidationException("Station name cannot be empty", stationType);
        }
        ...
        if (!station.matches(STATION_REGEX)) {
            throw new FieldValidationException("Station name must start with a Ukrainian capital letter and can include spaces, hyphens, numbers, and parentheses.", stationType);
        }
