...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Table</title>
    <link rel="icon" href="${pageContext.servletContext.contextPath}/png/favicon.png" sizes="32x32" type="image/png">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH"
          crossorigin="anonymous">
</head>
...
<div class="container">
    <h1 class="text-center mt-4 mb-4">Employee List</h1>
    <p class="text-center mb-4">This table displays the list of employees as of <fmt:formatDate value="${now}" pattern="yyyy-MM-dd" />.</p>
    <table class="table table-bordered table-hover">
        <thead class="table-success">
        <tr>
            <th>Name</th>
        ...
        <tbody>
        <c:forEach var="empl" items="${employeeList}" varStatus="status">
            <tr class="${status.index % 2 == 0 ? 'table-light' : 'table-success'}">
                <td>${empl.name}</td>
                <td>${empl.birthdayAsString}</td>
                <td>${empl.genderAsString}</td>
                <td>${empl.salaryAsCurrency}</td>
                <td>${empl.programLanguage}</td>
                <td>${empl.importance}</td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
